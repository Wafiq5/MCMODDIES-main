package com.mcmoddies.item.custom;
...
public class DiamondDetectorItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if(!context.getWorld().isClient()){
            ...
            if (!foundBlock){
                player.sendMessage(Text.literal("No Diamond found."));
            }
        }
    ...
    private void outputValuableCoordinates(BlockPos blockpos, PlayerEntity player, Block block) {
        player.sendMessage(Text.literal("Found " + block.asItem().getName().getString()), false);
    }

    private boolean isValuableBlock(BlockState state) {
        return  state.isIn(ModTags.Blocks.DIAMOND_DETECTOR_DETECTABLE_BLOCKS);
    }
}
